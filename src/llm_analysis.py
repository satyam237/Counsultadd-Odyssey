...
Provide ONLY the following key mandatory requirements, using exact quotes where available:

1. The minimum years requirement for Temporary Staffing experience
2. Required company background information submission
3. W-9 and Certificate of Insurance requirements (including CPA letter alternative)
4. Required company contact information
5. Insurance proof submission requirements
6. Prohibited business operations statement
7. Project Manager/Account Administrator requirement
8. HUB/M/W/DBE certification and Attachment A requirements

Format each requirement as a separate bullet point.
Use exact quotes from the document.
Do not add any additional commentary or explanations.
Do not use markdown formatting (like **text**).
"""
...
def check_requirements(state: State):
    ...
    try:
        ...
        company_data_path = Path("data/docs/company_data.txt")
        ...
        missing_sections = [
            section for section in required_sections if section not in company_text
        ]

        if missing_sections:
            raise ValueError(
                f"Missing required sections in company data: {missing_sections}"
            )
